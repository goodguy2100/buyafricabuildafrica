...
// Shared with the first-login wizard via @/lib/wizard-options.
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === "signin") {
        ...
        // Bulk-imported provisional accounts must finish first-login setup.
        try {
          const regs = await getMyRegistrationsFn();
          const first = regs[0];
          if (first && first.account_status === "provisional" && first.first_login) {
            await navigate({ to: "/first-login", replace: true });
            return;
          }
        } catch {
          // Fall through to the normal destination if the check fails.
        }
